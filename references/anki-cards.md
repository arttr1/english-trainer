...
**Never wrap the Example field in a `<div>` in the card template.** A second, separate bug: the custom model's templates originally used `<div class="example">{{Example}}</div>` to style it, and on AMGI (the user's iOS client) that made Example run together with the preceding field on the question side — no space, no line break, e.g. "does an okay job of Xсправляется с X приемлемоThis tokenizer...". Root cause, confirmed against AMGI's actual source (`Sources/AmgiCardWeb/NativeCardContent.swift` / `CardComplexity.swift` in antigluten/amgi): its native SwiftUI renderer detects block boundaries only at *closing* tags (`</div>`, `</p>`, `</center>`) and at `<br>`/`<hr>` — never at an *opening* tag like `<div class="...">`, so the boundary is silently lost. `anki_sync.py`'s `ensure_custom_model()` now uses plain `<br><br>` instead, which is on AMGI's allowlist and works in both directions — this was fixed once already for all 52 live notes by editing the model's templates directly (one template edit fixes every note using that model, no per-note migration needed). Keep templates br/hr-only; don't reintroduce `<div>`, `<span style=...>` layout tricks, or other block-level wrapping in this model's templates without checking against AMGI's allowlist first (`b, i, u, em, strong, sub, sup, span, div, p, br, hr, img, center` are tag-allowed, but only closing tags count as boundaries for div/p/center).

**Never put attributes on `<hr>` in the card template (no `id="answer"`, no `class=`).** A third bug, same root cause category: Anki's own default reversed-card templates conventionally write `<hr id="answer">` between `{{FrontSide}}` and the answer — cosmetic in the desktop/web renderer, used only for CSS targeting. But AMGI's regex for recognizing `<hr>` as a block boundary is `<hr\s*/?>`, which requires only whitespace (or nothing) before the closing `>` — `<hr id="answer">` doesn't match it at all, so AMGI treats the whole `{{FrontSide}}<hr id="answer">{{Back}}...` as one unbroken run and the answer runs straight into the front side with no space, e.g. "to accomplishвыполнить, достичь, осуществить". Fixed the same way — edited the model's templates via AnkiConnect to plain `<hr>` with no attributes, which fixed all existing notes on that model at once. `ensure_custom_model()` in `anki_sync.py` reflects this; don't reintroduce `id`/`class` on `<hr>` there.
